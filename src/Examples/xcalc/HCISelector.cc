...
template class HCISelector<u_char,u_short,float>;
                                      ...
                                      XVImage<Z> *output_image=NULL)
{
  ...
  for(i=0;i<image->Height();i++,pict+=image->Skip())
   for(j=0;j<image->Width();j++) *i_ptr++=distance[*pict++];
  output_image->unlock();
  return output_image;
}
			...
			int count,int& index, HCIObject& object)
{
   ...
   if(index>-1 && length>Sqr(SCALE(30)))
   {
     // did it change?
     if(abs(object.length-length)>Sqr(SCALE(20)))
     {
       object.moving=2,object.length=length;
