...
XVImageYCbCr::operator IM_TYPE() const { \
\
  typedef IM_TYPE::PIXELTYPE PIXTYPE; \
\
  IM_TYPE newIM; \
  if(newIM.ImageType() == this->ImageType()) newIM = dynamic_cast<IM_TYPE &>(const_cast<XVImageYCbCr & >(*this)); return newIM; \
  newIM.resize(this->Width() * 2, this->Height()); \
  XVImageIterator<XV_YCbCr> fromIter(*this); \
